"""
WikiText-103 data pipeline for HyViT language modelling experiments.
...
Subsequent calls load from cache in data/wikitext_cache/.
"""
...
_CACHE_DIR = Path(__file__).parent / "wikitext_cache"
...
def _load_split(split: str, seq_len: int) -> torch.Tensor:
    ...
    if cache_file.exists():
        try:
            ...
            print(f"Cache file corrupted, rebuilding: {cache_file}")
            cache_file.unlink()
    ...
    # Encode in batches to avoid building one massive string in memory
    BATCH = 512
    newline_tok = enc.encode_ordinary("\n")[0]
    all_tokens: list[int] = []
    batch: list[str] = []

    def _flush(b: list[str]) -> None:
        for toks in enc.encode_batch(b, num_threads=4):
            all_tokens.extend(toks)
            all_tokens.append(newline_tok)
