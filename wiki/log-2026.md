...
## [2026-07-21] ingest | Batch ingest: 4 new papers — Kimi Linear, LLaDA, Z-token Compressor, Gemma 4
- Sources ingested:
  - 2510.26692v2.pdf → Kimi Linear (Kimi Team, Moonshot AI, Nov 2025)
  - 2502.09992v3.pdf → LLaDA: Large Language Diffusion Models (Nie et al., Renmin U/Ant Group, NeurIPS 2025)
  - 2603.25340v2.pdf → LLM as Token Compressor and Decompressor (Li et al., May 2026)
  - 2607.02770v1.pdf → Gemma 4 Technical Report (Google DeepMind, Jul 2026)
- New raw source files:
  - wiki/raw/papers/2510.26692v2.md
  - wiki/raw/papers/2502.09992v3.md
  - wiki/raw/papers/2603.25340v2.md
  - wiki/raw/papers/2607.02770v1.md
- New entity pages:
  - wiki/entities/kimi-linear.md — Kimi Delta Attention (channel-wise gated delta rule), 3:1 hybrid with MLA + NoPE; 75% KV cache reduction, 6.3× TPOT at 1M context; first linear-hybrid to beat full attention
  - wiki/entities/llada.md — from-scratch 8B masked-diffusion LM; matches AR at scale; beats GPT-4o on reversal reasoning; no KV caching
  - wiki/entities/z-token-compression.md — LoRA-adapted LLM as content-adaptive discrete-token (Z-token) compressor/decompressor; beats ICAE/AutoCompressor/Gist Token/LLOCO
  - wiki/entities/gemma-4.md — open-weight multimodal dense/MoE family; encoder-free 12B; KV cache sharing; MTP drafter; QAT across LLM/vision/audio; top open dense model on Arena
- Pages updated:
  - wiki/concepts/kv-cache.md — added Kimi Linear and Gemma 4 to Architectural Reduction; added z-token-compression as a complementary axis; updated sources, See Also, date
  - wiki/concepts/diffusion-language-models.md — added LLaDA as the foundational from-scratch full-scale DLM entry (Key Systems + See Also); updated sources, date
  - wiki/concepts/mixture-of-experts.md — added Gemma 4's 26B-A4B MoE variant to See Also; updated sources, date
  - wiki/concepts/speculative-decoding.md — added Gemma 4's MTP drafter to See Also; updated sources, date
  - wiki/concepts/quantization.md — added Multimodal Encoder Quantization section (Gemma 4 QAT on vision/audio); updated sources, See Also, date
- index.md updated: total pages 69 → 73; added gemma-4, kimi-linear, llada, z-token-compression entries; bumped date
